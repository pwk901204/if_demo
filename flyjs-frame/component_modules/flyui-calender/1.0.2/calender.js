...
var headerTemplate = fly.template(__inline('header.html'));

var bodyTemplate = fly.template(__inline('body.html'));

var footerTemplate = fly.template(__inline('footer.html'));
...
var Calender = (module.exports = fly.Component.extend({
  ...
  ctor: function (element, options) {
    ...
    var daystr =
      days.join('').substring(options.weekStart || 0, 7) +
      days.join('').substring(0, options.weekStart || 0);
    days = daystr.split('');
  ...
  _wrapper: function () {
    ...
    each(mode, function (i, item) {
      that[item] = $('<div class="calender-' + item + '" />')
  ...
  _sort: function (dates, desc) {
    ...
    each(dates, function (i, item) {
      ...
      if (item === '$now') {
        ...
        dateWidget =
          dateWidget.flyCalender ||
          dateWidget.flyDatePicker ||
          dateWidget.handler;
        if (dateWidget) {
  ...
  _createHeader: function (mode) {
    ...
    if (mode == 'days') {
      header
        .on(CLICK, '.next', proxy(that._nextMonth, that))
  ...
  _createBody: function (mode) {
    ...
    if (mode == 'times') {
      ...
      body.on(
        CLICK,
        'td.' + itemName + ':not(.disabled)',
        proxy(that['_select' + toUpperCase(itemName)], that)
      );
      if (mode == 'years') {
        body
          .on(CLICK, '.next span', proxy(that._nextYears, that))
  ...
  _fillYears: function () {
    ...
    viewYear =
      parseInt(
        !isDivided ? (viewYear - 1) / displayNumber : viewYear / displayNumber,
        10
      ) * displayNumber;
  ...
  _getTime: function () {
    ...
    if (format.indexOf('mm') > -1)
      time += ':' + this._parseTime(date.getMinutes());
    if (format.indexOf('ss') > -1)
      time += ':' + this._parseTime(date.getSeconds());
    return time;
  },
  ...
  _createTimes: function (mode) {
    ...
    for (; i < total; i++) {
      ...
      isDisabled =
        targetTime.valueOf() + modeTime[mode] <= that._min() ||
          targetTime.valueOf() > that._max()
          ? ' ' + DISABLED
          : '';
      html +=
        '<li class="' +
  ...
  _selectTime: function (e) {
    var $time = $(e.target),
      ...
      type = 'hours';
    if ($time.hasClass('minute')) {
      type = 'minutes';
    } else if ($time.hasClass('second')) {
      type = 'seconds';
    }

    if ($time.hasClass('disabled') || $time.hasClass('active')) {
      return;
    }
    ...
    // 判断在maxDate的前提下，其他是否可选
    // 当前时小于最大时，分秒不做限制，均可选
    //修复1.0.1缺陷，1.0.1版本如果没有设置最大值最小是，会出现NAN。
    var max = this._max(),
      min = this._min();
    if (max === Infinity) {
      var maxHour = 24,
      ...
      var maxCurrent = new Date(max),
        maxHour = maxCurrent.getHours(),
        maxMinute = maxCurrent.getMinutes(),
        maxSecond = maxCurrent.getSeconds();
    }
    if (min === -Infinity) {
      var minHour = 0,
        minMinute = 0,
      ...
      var minCurrent = new Date(min),
        minHour = minCurrent.getHours(),
        minMinute = minCurrent.getMinutes(),
        minSecond = minCurrent.getSeconds();
    }
    var $hour = null,
      $minute = null,
      $second = null;
    ...
    if (type === 'hours') {
      $hour = $time.closest('td');
      $minute = $time.closest('td').next();
      $second = $minute.next();

      if (time < maxHour && (!minHour || time > minHour)) {
        $minute.find('li').removeClass('disabled');
        $second.find('li').removeClass('disabled');
      } else {
        $minute.find('li:gt(' + (maxMinute - 1) + ')').addClass('disabled');
        $minute.find('li:lt(' + (minMinute - 1) + ')').addClass('disabled');
        $minute
          .find('li:eq(' + maxMinute + ')')
          .removeClass('disabled')
          .addClass('active')
          .siblings()
          .removeClass('active');
        this.viewDate['setMinutes'](maxMinute);
        this.date['setMinutes'](maxMinute);

        $second.find('li:gt(' + (maxSecond - 1) + ')').addClass('disabled');
        $second.find('li:lt(' + (minSecond - 1) + ')').addClass('disabled');
        $second
          .find('li:eq(' + maxSecond + ')')
          .removeClass('disabled')
          .addClass('active')
          .siblings()
          .removeClass('active');
        this.viewDate['setSeconds'](maxSecond);
        this.date['setSeconds'](maxSecond);
      }
    }

    if (type === 'minutes') {
      $minute = $time.closest('td');
      $second = $minute.next();
      if (time < maxMinute && (!minMinute || time > minMinute)) {
        $second.find('li').removeClass('disabled');
      } else {
        $second.find('li:gt(' + (maxSecond - 1) + ')').addClass('disabled');
        $second.find('li:lt(' + (minSecond - 1) + ')').addClass('disabled');
  ...
  _createDateFromString: function (value) {
    ...
    switch (args.length) {
      case 1:
        return String(args[0]).length === 4
          ? new Date(args[0], 1, 1)
          : new Date(args[0]);
      case 2:
        return new Date(args[0], args[1] - 1, 1); // 只指定年月，默认补全天为1
      case 3:
        return new Date(args[0], args[1] - 1, args[2]);
      case 4:
        return new Date(args[0], args[1] - 1, args[2], args[3], 0, 0, 0);
      case 5:
        return new Date(args[0], args[1] - 1, args[2], args[3], args[4], 0, 0);
      case 6:
        return new Date(
          args[0],
          args[1] - 1,
    ...
    switch (l) {
      case 1:
        return String(args[0]).length === 4
          ...
          : new Date(args[0]);
      case 2:
        return new Date(args[0], args[1], 1); // 只指定年月，默认补全天为1
      case 3:
        return new Date(args[0], args[1], args[2]);
      case 4:
        return new Date(args[0], args[1], args[2], args[3], 0, 0, 0);
      case 5:
        return new Date(args[0], args[1], args[2], args[3], args[4], 0, 0);
      case 6:
        return new Date(
          args[0],
          args[1],
