/*! flyui v1.0.4 | by huanzhang & ueteam | (c) 2017 iFlytek, Inc. | Licensed under  | 2017-09-15 15:09:19 GMT+0800 */
(function e(t, n, r) {
    function s(o, u) {
        if (!n[o]) {
            if (!t[o]) {
        ...
        function(require, module, exports) {
            ...
            var Alert = Widget.extend({
                ...
                ctor: function(element, options) {
                    ...
                    that.element.on('click', '.close.icon', function() {
                        that.destroy();
                    });
                },
        ...
        function(require, module, exports) {
            ...
            var Calender = Widget.extend({
                ...
                ctor: function(element, options) {
                    ...
                    var daystr = days.join('').substring(options.weekStart || 0, 7) +
                        days.join('').substring(0, options.weekStart || 0);
                    that._weekdays = daystr.split('');
                ...
                _textAccessor: function(text) {
                    ...
                    if (text !== undefined) {
                        if (isPlainObject(text) || text instanceof ObservableObject) {
                            span.html(template(text)).attr('title', template(text));
                        } else if ($.isArray(text) || text instanceof ObservableArray) {
                            allTexts = $.map(text, function(item, i) {
                                return template(item);
                            }).join(options.textSplit);
                            span.html(allTexts).attr('title', allTexts);
                        } else if (optionLabel && this._optionLabelText() === text) {
                            span.html(this.optionLabelTemplate(optionLabel)).attr('title', this.optionLabelTemplate(optionLabel));
                        }
                    } else {
                        return span.text();
                    }
    ...
    20: [
        function(require, module, exports) {
            ...
            "E:\\03.案例库\\flyui\\flyui1.0\\flyui-ex\\.build\\temp\\fly.accordion.js": 1
        }
    ],
    21: [
        function(require, module, exports) {
            ...
            var StaticList = ui.DataBoundWidget.extend({
                ...
                _renderItem: function(context) {
                    var item =
                        '<li tabindex="-1" role="option" unselectable="on" class="item ell';
                    ...
                    item += '"' + ' title="'+this.templates.template(dataItem)+'" data-offset-index="' + context.index + '">';
        ...
        function(require, module, exports) {
            ...
            var Pagination = Widget.extend({
                ...
                _wrapper: function() {
                    ...
                    if (options.showSwitch) {
                        that.switchView.html(
                            '<span class="mydx-span">每页</span> <input style="width:60px" type="input"> <span class="mydx-span">条记录</span>');
                ...
                setDataSource: function(dataSource) {
                    ...
                    if (that.options.showSwitch) {
                        that.switchPage.value(that.dataSource.pageSize());
                    }
                    that.selectPage();
                },
                ...
                _drawInfo: function() {
                    ...
                    if (this.options.showTotal && dataSource.options.total !== false) {
                        this.totalView.html('总' + dataSource.total() + '条记录，共' +
                ...
                _templates: function() {
                    ...
                    e.templates = {
                        ...
                        itemElement: templateNoWith(
                            ...
                            '{{ else }}' +
                            '<span class="{{ r.textClass($data.item) }}" title="{{# r.itemContent($data) }}">{{# r.itemContent($data) }}</span>' +
                            '{{/if}}' +
                            '</div>'
                        ),
                        item: templateNoWith(
    ...
    35: [
        function(require, module, exports) {
            ...
            var validate = function(ele, valid, hasTooltip) {
                ...
                ele.parent().toggleClass('errorWrap', !isPass);
