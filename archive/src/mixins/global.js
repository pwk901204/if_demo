export const globalMixin = {
  data () {
    return {
      PAGINATION: {
        total: 0,
        pageNo: 1,
        pageSize: 10
      },
      ADVANCED: false
    }
  },
  methods: {
    /**
    * 获取查询列表参数
    * @param { Form } form 表单对象
    * @param { Object } customParam 自定义参数
    */
    _getTableParams (form, customParam) {
      let params
      if (form && form._isVue) {
        const values = this._getFieldsValue(form)

        params = { ...values, ...this.PAGINATION, ...customParam }
      } else {
        customParam = form
        params = { ...this.PAGINATION, ...customParam }
        // debugger
      }
      delete params.total
      return params
    },

    /**
    * 获取查询列表参数
    * @param { Form } form 表单对象
    * @param { Object } customParam 自定义参数
    */
    _getFieldsValue (form) {
      const values = form.getFieldsValue()
      const results = JSON.parse(JSON.stringify(values))

      for (const key in values) {
        const val = values[key]
        if (val) {
          if (val._isAMomentObject) {
            const node = document.getElementById(key)
            const fields = node.getAttribute('data-key') || key
            const formatStr = node.getAttribute('data-format') || 'YYYY-MM-DD' // 格式类型

            results[fields] = val.format(formatStr)
          }

          if (val.constructor === Array && val.length > 0 && val[0]._isAMomentObject) {
            const node = document.getElementById(key)
            let fields = node.getAttribute('data-key') || `${key}StartTime,${key}EndTime`
            if (fields && fields.indexOf(',') > 0) {
              const formatStr = node.getAttribute('data-format') || 'YYYY-MM-DD' // 格式类型
              fields = fields.split(',')

              results[fields[0]] = val[0].format(formatStr)
              results[fields[1]] = val[1].format(formatStr)
              delete results[key]
              continue
            }
          }

          // 处理多选数组类型，逗号分隔
          if (val.constructor === Array) {
            results[key] = val.join(',')
          }
        }
        // 未赋值的取值
        if (val === undefined) {
          delete results[key]
        }
      }
      return results
    },

    /**
     * 表单赋值
     * @param { Form } form 表单对象
     * @param { Object } values 表单值
     * @param { Object } formatRule 格式规则
     */
    _setFieldsValue (form, values, formatRule) {
      const fields = form.getFieldsValue()
      for (const key in fields) {
        const value = values[key]
        if (value !== undefined && value !== null) {
          if (formatRule) {
            if (formatRule[key] === 'Moment') {
              fields[key] = value ? window.moment(value) : undefined
            } else if (formatRule[key] === 'Array') {
              fields[key] = value.split(',')
            } else {
              fields[key] = value
            }
          } else {
            fields[key] = value
          }
        }
      }

      form.setFieldsValue(fields)
    }
  }
}

// export const tableMixin = {
//   data () {
//     return {
//       PAGINATION: {
//         total: 0,
//         pageNo: 1,
//         pageSize: 10
//       }
//     }
//   },
//   methods: {
//     /**
//     * 获取查询列表参数
//     * @param { Form } form 表单对象
//     * @param { Object } customParam 自定义参数
//     */
//     _getTableParams (form, customParam) {
//       let params
//       if (form && form._isVue) {
//         const values = this._getFieldsValue(form)

//         params = { ...values, ...this.PAGINATION, ...customParam }
//       } else {
//         customParam = form
//         params = { ...this.PAGINATION, ...customParam }
//         // debugger
//       }
//       delete params.total
//       return params
//     }
//   }
// }

// export const formMixin = {
//   methods: {
//     /**
//     * 获取查询列表参数
//     * @param { Form } form 表单对象
//     * @param { Object } customParam 自定义参数
//     */
//     _getFieldsValue (form) {
//       const values = form.getFieldsValue()
//       const results = JSON.parse(JSON.stringify(values))

//       for (const key in values) {
//         const val = values[key]
//         if (val) {
//           if (val._isAMomentObject) {
//             const node = document.getElementById(key)
//             const fields = node.getAttribute('data-key') || key
//             const formatStr = node.getAttribute('data-format') || 'YYYY-MM-DD' // 格式类型

//             results[fields] = val.format(formatStr)
//           }

//           if (val.constructor === Array && val.length > 0 && val[0]._isAMomentObject) {
//             const node = document.getElementById(key)
//             let fields = node.getAttribute('data-key') || `${key}StartTime,${key}EndTime`
//             if (fields && fields.indexOf(',') > 0) {
//               const formatStr = node.getAttribute('data-format') || 'YYYY-MM-DD' // 格式类型
//               fields = fields.split(',')

//               results[fields[0]] = val[0].format(formatStr)
//               results[fields[1]] = val[1].format(formatStr)
//               delete results[key]
//               continue
//             }
//           }

//           // 处理多选数组类型，逗号分隔
//           if (val.constructor === Array) {
//             results[key] = val.join(',')
//           }
//         }
//         // 未赋值的取值
//         if (val === undefined) {
//           delete results[key]
//         }
//       }
//       return results
//     },

//     /**
//      * 表单赋值
//      * @param { Form } form 表单对象
//      * @param { Object } values 表单值
//      * @param { Object } formatRule 格式规则
//      */
//     _setFieldsValue (form, values, formatRule) {
//       const fields = form.getFieldsValue()
//       for (const key in fields) {
//         const value = values[key]
//         if (value !== undefined && value !== null) {
//           if (formatRule) {
//             if (formatRule[key] === 'Moment') {
//               fields[key] = value ? window.moment(value) : undefined
//             } else if (formatRule[key] === 'Array') {
//               fields[key] = value.split(',')
//             } else {
//               fields[key] = value
//             }
//           } else {
//             fields[key] = value
//           }
//         }
//       }

//       form.setFieldsValue(fields)
//     }
//   }
// }
