...
function WebOffice2015() {
	this.SaveServlet = "/webOffice/saveWord";
  ...
  // 自定义参数 end
	...
	this.MaxFileSize = 200 * 1024; 			// 设置文件最大允许值，单位k，默认为8M
    ...
    this.WebSetMsgByName("projectId", this.projectId);
    var httpclient = this.obj.Http; // 设置http对象
    httpclient.AddForm("FormData", this.GetMessageString());

		//alert("want to download: " + downloadLink);
    console.log('start...open..word..'+ downloadLink)
		if (httpclient.Open(this.HttpMethod.Post, downloadLink, false))
        {
          console.log('httpclient.Open.#####'+ downloadLink)
            if (httpclient.Send())
            {
              console.log('httpclient.Send.#####'+ downloadLink)
            	if (this.hiddenSaveLocal(httpclient, this, false, false)) {
            	  console.log('hiddenSaveLocal...true')
					var mSaveResult = this.WebOpenLocalFile(this.DownloadedFileTempPathName);
                console.log('mSaveResult...'+ mSaveResult)
					if (mSaveResult == 0) { // 打开本地磁盘文件
						this.getOfficeVersion();// 打开文档后，判断当前office版本
						return true;
					}else if(mSaveResult == 1){
...
//	   	};
	...
	this.WebSave = function()
	{
		...
		httpclient.Clear();
		// 自定义参数start
    this.WebSetMsgByName("projectId", this.projectId);  // 项目id
    this.WebSetMsgByName("userId", this.userId); //  用户id
      ...
      console.log("mSavePath...:"+ mSavePath);
    ...
    this.WebSaveTemplate = function()
    {
    	...
    	var httpclient = this.obj.Http; //设置http对象
    	httpclient.Clear();
      this.WebSetMsgByName("templateId", this.templateId);
      this.WebSetMsgByName("userId", this.userId);
      this.WebSetMsgByName("FILENAME", this.FileName);
    ...
    var tempFilePath = this.getFilePath() + Math.random() * 100000 + this.iWebOfficeTempName;
		this.WebDelLocalFile(tempFilePath);	//删除临时文件
    	var mSaveResult = this.WebSaveLocalFile(tempFilePath);//将当前文档保存下来
    	if (!(mSaveResult == 0)){
    		this.Status = "保存本地文档失败！错误代码为：" + mSaveResult;
    		return false;
    	}
    	...
    	if(this.LOADFILE(httpclient)){
    		if(this.VBAInsertFile("Content",tempFilePath)){
    		  // 删除临时文件
          this.WebDelLocalFile(tempFilePath);
    			this.Status = "套红成功";
    			return true;
    		}else{
		...
		if(this.FileType == ".doc" || this.FileType == ".docx"){
			if(this.obj.ActiveDocument.ProtectionType == "-1"){
				...
				if(httpclient.Open(this.HttpMethod.Post, this.WebUrl, false)){
					if(httpclient.Send()){
						if (httpclient.Status == 200) {
             var imgName = this.GetSingleFileName(this.ImageName);
             console.log(imgName);
							if(this.hiddenSaveLocal(httpclient,this,false,false,imgName)){
...
/*	 文档保存到本地（有对话框）
  ...
  /* 在光标处添加文本 */
  this.addSelectContent = function(txt,fColor){

    // var uuid = this.kg_uuid();
    // this.WebAddBookMarks(uuid,"");

    var Selection = this.WebObject.ActiveDocument.Application.Selection;
    // Selection.Font.Color = fColor; //字体红色 这里的颜色是用的是RGB 数字
  ...
  this.VBAProtectDocument = function(docType, password)
  {
	...
	this.VBAUnProtectDocument = function(docType, password)
	{
      ...
      for(var i=1;i<=sCount;i++){
        if(this.WebObject.ActiveDocument.Application.ActiveWorkbook.Sheets.Item(i).ProtectContents){
          this.WebObject.ActiveDocument.Application.Sheets.Item(i).Unprotect(password);
        }
      }
			this.Status = "解除保护";
			return true;
    ...
    */
	...
	this.SAVEFILE = function(httpclient, FileName) // 与后台发生交互OPTION值为：SAVEFILE
	{
		...
		this.WebClearMessage();
		httpclient.ShowProgressUI = this.ShowWindow; // 隐藏进度条
		this.WebUrl = this.ServerUrl + this.SaveServlet;   // "http://localhost:8888/iWebOffice2015.JSP-V2.1/OfficeServer"
		...
		if (httpclient.Open(this.HttpMethod.Post, this.WebUrl, false)) //true 异步方式 false同步
		{
			if (!httpclient.Send()) {
				...
				this.Status = httpclient.GetResponseHeader("STATUS");
        this.responseFilePath = httpclient.GetResponseHeader("filePath");
        console.log(this.responseFilePath);
				return true;
			}
		} else {
	...
	this.hiddenSaveLocal = function(httpclient, webOffice, isHidden,
		...
		try {
			...
			console.log(this.DownloadedFileTempPathName);
			httpclient.Clear();
			return true;
		} catch (e) {
			return false;
	...
	{
		var getVersion = 0.0;
    console.log(getVersion + '' +this.setVersion)
		try {
